...
const AttendanceComparision = ({
  correction,
}: {
  correction: AttendanceCorrectionResponse | null;
}) => {
  if (!correction)
    ...
    <>
      <Table>
        ...
        <TableBody>
          ...
          {(correction?.previous?.status || correction?.changes?.status) && (
            <TableRow>
              ...
              <TableCell>
                <Badge
                  className="px-3 py-1"
                  variant={attendanceStatusVariant[correction?.previous.status]}
                >
                  {correction.previous.status}
                </Badge>
              </TableCell>
              <TableCell>
          ...
          {/* Status */}
          {(correction?.previous?.isLate !== undefined ||
            correction?.changes?.isLate !== undefined) && (
            <TableRow>
              <TableCell>Late</TableCell>
              <TableCell>
                {correction.previous.isLate ? (
                  <Badge className="px-3 py-1" variant={"outline-warn"}>
                    Late
                  </Badge>
                ) : (
                  <Badge className="px-3 py-1" variant={"outline-success"}>
                    On Time
                  </Badge>
                )}
              </TableCell>
