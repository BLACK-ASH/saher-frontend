      ...
      <CardContent className="flex max-md:flex-col flex-row ">
                    ...
                    <SelectGroup>
                      {
                        roles.map((role) => {
                          return (
                            <SelectItem key={role} value={role}>{role.toLocaleUpperCase()}</SelectItem>
                          )
                        })
                      }
                    </SelectGroup>
        ...
        <FieldGroup className="w-1/2 m-2 max-md:w-full">
                    ...
                    <SelectGroup>
                      {
                        genders.map((gender) => {
                          return (
                            <SelectItem key={gender} value={gender}>{gender.toLocaleUpperCase()}</SelectItem>
                          )
                        })
                      }
                    </SelectGroup>
                ...
                <Popover open={open} onOpenChange={setOpen}>
                  ...
                  <PopoverContent className="w-auto overflow-hidden p-0" align="start">
                    <Calendar
                      ...
                      onSelect={(date) => {
                        form.setValue("account.dateOfBirth", date?.toDateString() || "")
                        setOpen(false)
                      }}
                    />
                  </PopoverContent>
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldLabel htmlFor="secondary-phone-number">Secondary Phone Number</FieldLabel>
                <Input
                  {...field}
