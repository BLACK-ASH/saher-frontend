...
| 02-03-T2 | 03 | 1 | FNDT-06 | T-2-06 | manager-nav fix; can()-only gating; RoleAccess allow()-contract sweep — intern denied everywhere (render proof) | render + typecheck | `pnpm test components/sidebar/nav-list.test.tsx components/role-access.test.tsx` | ❌ co-located | ⬜ pending |
...
| 02-06-T1 | 06 | 3 | FNDT-05/02 | T-2-15 | TanStack pageCount normalized; footer wired — incl. users-admin, generic holiday DataTable, and mail tables (client-side recipe); dashboard raw parses pre-declared to 02-07-T2 | full gates + grep | `pnpm test && pnpm build && ! grep -n "Previous" features/users/data-table.tsx components/data-table.tsx features/mail/data-table.tsx` | ✅ | ⬜ pending |
| 02-06-T2 | 06 | 3 | FNDT-03 (D-21) / D-02 | T-2-15/16 | six hooks → one factory; prefix invalidation; four program-family headers onto PaginationFooter fed by normalized totalPages (next-boundary guard restored) | full gates + grep | `pnpm lint && pnpm typecheck && pnpm test && pnpm build && ! grep -rn "ArrowLeft\|ArrowRight" features/program/*/[a-z]*-header.tsx` | ✅ | ⬜ pending |
...
| 02-07-T1 | 07 | 4 | FNDT-02 | — | swept display surfaces render canonical IST formats; zero toLocale*String in swept dirs incl. calendar + attendance-chart (both its raw sites incl. :54 parse) | gates + grep | `pnpm lint && pnpm typecheck && pnpm build && ! grep -rEn "toLocale(Date\|Time)?String\(" --include="*.tsx" --include="*.ts" app/\(main\)/mail app/\(main\)/calendar features/leave features/holiday features/profile features/attendance features/dashboard app/\(main\)/program` | ✅ | ⬜ pending |
| 02-07-T2 | 07 | 4 | FNDT-02 (D-10/D-12) | T-2-18/19 | date-only helper boundary round-trips; session-editor sends +05:30 ISO; pre-declared dashboard raw parses (range-attendance-table:67,92,109 / attendance-dashboard:19,27-28,122-140) converted onto IST helpers | unit + gates + grep | `pnpm test lib/date.test.ts && pnpm lint && pnpm typecheck && ! grep -rEn "new Date\([^)]" features/dashboard/range-attendance-table.tsx features/dashboard/attendance-grid/attendance-dashboard.tsx && pnpm build` | ❌ co-located w/ task | ⬜ pending |
